# JournalLM V2 — specification index

Detailed specs live **one Markdown file per implementation step**, aligned with [JOURNALLM_V2_IMPLEMENTATION_PLAN.md](../../JOURNALLM_V2_IMPLEMENTATION_PLAN.md).

| Step | Document | Status |
|------|----------|--------|
| 0 | [00-versioning-and-conventions.md](./00-versioning-and-conventions.md) | Complete |
| 1 | [01-database-foundation.md](./01-database-foundation.md) | Complete |
| 2 | [02-shredder-v2.md](./02-shredder-v2.md) | Stub |
| 3 | [03-entity-resolution.md](./03-entity-resolution.md) | Stub |
| 4 | [04-entity-inbox-api.md](./04-entity-inbox-api.md) | Stub |
| 5 | [05-entity-inbox-ui.md](./05-entity-inbox-ui.md) | Stub |
| 6 | [06-backfill-and-operations.md](./06-backfill-and-operations.md) | Stub |
| 7 | [07-dashboard-data-layer.md](./07-dashboard-data-layer.md) | Stub |
| 8 | [08-dashboard-editorial-ui.md](./08-dashboard-editorial-ui.md) | Stub |
| 9 | [09-embeddings-and-rag.md](./09-embeddings-and-rag.md) | Stub |
| 10 | [10-people-and-projects-pages.md](./10-people-and-projects-pages.md) | Stub |
| 11 | [11-entity-aware-chat-retrieval.md](./11-entity-aware-chat-retrieval.md) | Stub |
| 12 | [12-coach-chat-mode.md](./12-coach-chat-mode.md) | Stub |
| 13 | [13-release-and-hardening.md](./13-release-and-hardening.md) | Stub |

**Product intent:** [JOURNALLM_V2_IDEATION.md](../../JOURNALLM_V2_IDEATION.md)

Each step doc should link back to the ideation subsection IDs it implements (e.g. §1.5, §2.5).
