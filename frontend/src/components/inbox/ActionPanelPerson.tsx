...
import type {
  ProposalDetail,
  ConfirmNewPersonBody,
  MergePersonBody,
  DismissBody,
  BlocklistBody,
} from "@/lib/inbox-api";
import ConfirmNewPersonForm from "./ConfirmNewPersonForm";
import MergeForm, { type MergeSubmitBody } from "./MergeForm";
import ActionTabs from "./ActionTabs";
import ConfirmDialog from "@/components/chat/ConfirmDialog";

type PersonTab = "confirm" | "merge" | "dismiss" | "blocklist";

export type ActionPanelPersonProps = {
  ...
  onConfirmNew: (body: ConfirmNewPersonBody) => void;
  onMerge: (body: MergePersonBody) => void;
  onDismiss: (body: DismissBody) => void;
  onBlocklist: (body: BlocklistBody) => void;
  onSwitchToMerge: () => void;
};

export default function ActionPanelPerson({
  proposal,
  selectedTargetId,
  onSelectTarget,
  submitting,
  ...
  return (
    <div className="rounded-xl border border-[var(--color-brand-border)] bg-[var(--color-brand-surface)] p-5">
      ...
      <ConfirmDialog
        ...
        message='The same surface may re-appear in future shred runs. Use "Blocklist" if you want to silence it permanently.'
